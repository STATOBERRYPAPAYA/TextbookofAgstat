...
knitr::include_graphics("images/crdnumbered.png")
```
...
knitr::include_graphics("images/sequence.png")
```  
...
knitr::include_graphics("images/layoutcrd.png")
```  

### Statistical model for CRD  

Consider a CRD with *v* treatments with *r* replications. Statistical model for CRD is same as that of one-way ANOVA (@sec-oneway).  
...
Analysis of CRD is same as that explained in section @sec-oneway of one-way ANOVA.  
...
knitr::include_graphics("images/crdanova.png")
``` 
...
#| label: fig-crdex
#| fig-cap: "ANOVA table of CRD"
knitr::include_graphics("images/crdex.png")
```  
#### Solution  
...
```{r echo=FALSE, out.width="90%", fig.align='center'}
#| label: fig-crdanovaex
#| fig-cap: "ANOVA table of CRD"
knitr::include_graphics("images/crdanovaex.png")
```  
...
section @sec-ftable. It is shown as $t_{\frac{\alpha}{2}}$ , since
...
In the above table we can see that difference between mean of $V_{2}$ and $V_{1}$
is 4 which is greater than C.D. value. So $V_{2}$ and $V_{1}$ are significantly
...
So, in our example we can conclude that $V_{2}$ is the best variety because,
...
#| label: fig-groupingcrd
#| fig-cap: "A hypothetical example of treatment grouping"
knitr::include_graphics("images/groupingcrd.png")
```  

>If a lettering like this appears meaning is that those treatments having common alphabets are not significantly different. In the above table $V_{2}$
and $V_{1}$ is not significantly different as both of them are having common
letter *a*. At the same time you can see $V_{5}$ and $V_{4}$ is significantly
...
Block Design) blocks are of equal size and contains all the treatments. Since the block contains all the treatments, the term 'complete block design' is used. See figure @fig-field and @fig-field2 to see how blocks are formed in a field.
...
knitr::include_graphics("images/rcbd1.png")
``` 
...
knitr::include_graphics("images/rcbd2.png")
``` 
...
knitr::include_graphics("images/rcbd3.png")
``` 

### Statistical Model for RCBD

Consider a RCBD with $v$ treatments with $r$ replications (Blocks). Statistical model for RCBD is same as that of two-way ANOVA described in section @sec-twoway.  
...
### Analysis of RCBD  
Analysis of RCBD is same as that explained in section @sec-twoway of two-way ANOVA.

Observation from $v$ treatments in $r$ blocks is arranged as shown below.  

```{r echo=FALSE, out.width="70%", fig.align='center'}
#| label: fig-rcbd4
#| fig-cap: "Two-way arrangement of observations in RCBD"
knitr::include_graphics("images/rcbd4.png")
```  

The anova table following the calculations as in section @sec-twoway is  
```{r echo=FALSE, out.width="90%", fig.align='center'}
#| label: fig-rcbd5
#| fig-cap: "ANOVA table of RCBD"
knitr::include_graphics("images/rcbd5.png")
```
$F_{1}$ \~ F ~($v$--1),\ ($v$--1)($r$--1)~ and $F_{2}$ \~ F ~($r$--1),($v$--1)($r$--1)~ are two F values, one for treatment and one for blocks respectively. Decisions on the null hypothesis is made by comparing corresponding F value to the table value as explained in section @sec-ftable.  

> What if $F_{2}$ is not significant?  
> If $F_{2}$ is not significant, it indicates that there is no significant difference between blocks, means blocking is not effective. Blocking is considered effective in reducing the experimental error if $F_{2}$ is significant.  

### Number of replications in RCBD  

As explained in section @sec-replical number of replications is RCBD is found out using the formula of error degrees of freedom (*edf*).  
...
knitr::include_graphics("images/lsdlayout.png")
``` 
...
knitr::include_graphics("images/lsdlayout1.png")
``` 
...
knitr::include_graphics("images/lsdlayout2.png")
``` 
...
To calculate the sum of squares, arrange the data according to the rows, columns and treatments and obtain the respective totals. Let the treatment totals be $T_{1}$,$T_{2}$, .... $T_{v}$; Block totals be $B_{1}$, $B_{2}$, $B_{3}$, ..., $B_{v}$ and Column totals be $C_{1}$, $C_{2}$, $C_{3}$, ... $C_{v}$. 
```{r echo=FALSE, out.width="90%", fig.align='center'}
#| label: fig-lsdanova
#| fig-cap: "ANOVA of LSD"
knitr::include_graphics("images/lsdanova.jpg")
``` 
...
#| label: fig-LSDE3
#| fig-cap: "ANOVA"
knitr::include_graphics("images/LSDE3.png")
``` 
...
**"The object of statistical methods is the reduction of data." - R. A. Fisher**
:::