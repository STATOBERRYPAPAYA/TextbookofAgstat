...
```{r setup-mathfix, include=FALSE}
# Helper: convert math delimiters so MathJax reliably renders math inside
# kable table cells (works even when the math is followed by text such as
# "=90", and when a cell holds more than one math span). Display math
# $$...$$ becomes \[...\] and inline math $...$ becomes \(...\). Empty CSV
# cells (read by R as NA) are shown as blanks rather than "NA". The CSV
# data on disk is left completely untouched.
fix_math <- function(x) {
  x <- as.character(x)
  x[is.na(x)] <- ""                                                # blank out NA
  x <- gsub("\\$\\$(.+?)\\$\\$", "\\\\[\\1\\\\]", x, perl = TRUE)  # display math
  x <- gsub("\\$([^$]+)\\$", "\\\\(\\1\\\\)", x, perl = TRUE)       # inline math
  x
}
```
...
dt %>%
  kbl(escape = FALSE, booktabs = FALSE, align = "c") %>%
  kable_classic(full_width = F, html_font = "Cambria", lightable_options = "basic") %>%
