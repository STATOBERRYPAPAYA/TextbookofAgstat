...
::: {#exm-ct13}
If the weight of sorghum ear heads are 45, 60, 48, 100, 65 gms. Find the geometric mean?
...
::: {#exm-ct15}
Geometric mean of a grouped frequency distribution
...
::: {#exm-ct17}
For the given data calculate the harmonic mean.
...
::: {#exm-ct18}
*Cistern Problem*-Two pipes are used to fill a cistern. Pipe A can fill the cistern in 3 hours. Pipe B can fill the cistern in 5 hours. If both pipes are opened at the same time, how long will it take to fill the cistern completely?
...
::: {#exm-ct19}
A car travels a certain distance from City A to City B at a speed of 60 km/h, and returns the same distance from City B to City A at a speed of 90 km/h. What is the average speed for the entire trip?
...
![Arithmetic mean on a semi circle](images/central2/semiam.svg){#fig-semiam fig-align="center" width="50%" style="text-align:center;"}
...
![Geometric mean on a semi circle](images/central2/semigm.svg){#fig-semigm fig-align="center" width="50%" style="text-align:center;"}
...
![Geometric mean and aritmetic mean on a semi circle](images/central2/semiamgm.svg){#fig-semiamgm fig-align="center" width="50%" style="text-align:center;"}
...
![AM, GM and HM on a semi circle](images/central2/semiamgmhm.svg){#fig-semiamgmhm fig-align="center" width="50%" style="text-align:center;"}
...
::: {#exm-ct22}
Find the quartiles for the grouped frequency data given
...
::: {#exm-ct23}
Compute ${P}_{25}$ and ${P}_{75}$ for the data 25, 18, 30, 8, 15, 5, 10, 35, 40, 45.
...
::: {.callout-important title="Note"}
Data in @exm-ct23 is the same as @exm-ct20; it can be seen that $P_{25} = Q_{1}$ & $P_{75} = Q_{3}$ always
:::

::: {#exr-ct1}
Find $P_{25}$, $P_{50}$ & $P_{75}$ for @exm-ct21 & @exm-ct22; verify that $P_{50} = Q_{2}$, $P_{25} = Q_{1}$ & $P_{75} = Q_{3}$
:::
...
::: {#exr-ct2}
Find $\text{D}_{5}$ for @exm-ct21, @exm-ct22 & @exm-ct23; verify that ${D}_{5} = {Q}_{2} = {P}_{50} = median$
:::
...
`1.` Geometric mean; Harmonic mean `2.` Growth rates `3.` $n$th `4.` $GM$ `5.` Antilog `6.` $f_i\log x_i$ `7.` Zero `8.` Odd `9.` Reciprocals `10.` Rates `11.` $HM$ `12.` Four `13.` $Q_1$ `14.` $Q_2$; Median `15.` $Q_3$ `16.` 50 `17.` 25 `18.` 75 `19.` Hundred `20.` Median `21.` First `22.` Third `23.` Ten `24.` Nine `25.` Median `26.` Quartile `27.` Percentile `28.` Decile `29.` Pythagorean `30.` $\geq$; $\geq$
:::
...
12. Using @eq-percentile1 on the frequency distribution from @tbl-quartile (@exm-ct21, $n=24$): $P_{25}=8=Q_1$, $P_{50}=15=Q_2=\text{Median}$, $P_{75}=24=Q_3$, verifying the relationship.
