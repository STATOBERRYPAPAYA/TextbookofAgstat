...
Consider an experiment with $v$ treatments with $r$ replications each. The observations are recorded as shown in @tbl-oneway.

```{r tbl-oneway,echo=FALSE,warning=FALSE,results='asis'}
#| tbl-cap: "One-way classification of data"
...
library(knitr)
library(kableExtra)
dt <- read.csv("csv/oneway_classification.csv", check.names = FALSE)
colnames(dt) <- c("Treatments", "", "", "", "", "", "Total")
dt %>%
  ...
  add_header_above(c(" " = 1, "Observations" = 5, " " = 1), bold = TRUE)
```
...
|:-------|:---:|:----|:------|:--|
| Between treatments | $v-1$ | $\dfrac{\sum_{i=1}^{v} T_i^2}{r} - \text{C.F.} = \text{SST}$ | $\dfrac{\text{SST}}{v-1} = \text{MST}$ | $F = \dfrac{\text{MST}}{\text{MSE}}$ |
| Within treatments (Error) | $v(r-1)$ | $\text{TSS} - \text{SST} = \text{SSE}$ | $\dfrac{\text{SSE}}{v(r-1)} = \text{MSE}$ | |
| Total | $vr-1$ | $\sum_{i=1}^{v}\sum_{j=1}^{r} Y_{ij}^2 - \dfrac{G^2}{n} = \text{TSS}$ | | |

: One-way ANOVA table {#tbl-onewayanova .bordered}
...
The data are arranged as shown in @tbl-onewayex.

```{r tbl-onewayex,echo=FALSE,warning=FALSE,results='asis'}
#| tbl-cap: "One-way classified data on tiller count"
...
library(knitr)
library(kableExtra)
dt <- read.csv("csv/oneway_data.csv", check.names = FALSE)
dt %>%
  kbl(booktabs = TRUE, align = "c", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Georgia") %>%
  kable_styling(
  ...
  add_header_above(c(" " = 1, "4 observations from each treatment" = 4, " " = 1), bold = TRUE)
```
...
```{r tbl-onewayex1,echo=FALSE,warning=FALSE,results='asis'}
#| tbl-cap: "ANOVA calculations for the tiller count data"
#| fig-cap: "ANOVA calculations for the tiller count data"
#| filters: 
#| - parse-latex
...
library(knitr)
library(kableExtra)
dt <- read.csv("csv/oneway_calc.csv", check.names = FALSE)
dt %>%
  kbl(booktabs = TRUE, align = "c", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Georgia") %>%
  kable_styling(
  ...
  add_header_above(c(" " = 1, "4 observations from each treatment" = 4, " " = 3), bold = TRUE)
```

The sum of the last column gives $\sum_{i=1}^{6} \dfrac{T_i^2}{r_i} = 109372.25$.
...
|:-------|:---:|:---:|:------|:-------:|:-------:|
| Between treatments | $6-1=5$ | 10427.21 | $\dfrac{10427.21}{5}=2085.442$ | $\dfrac{2085.442}{162.93}=12.7996^{**}$ | $F_{(5,18)}=2.77$ |
| Within treatments (Error) | $6(4-1)=18$ | 2932.74 | $\dfrac{2932.74}{18}=162.93$ | | |
| Total | $24-1=23$ | 13359.95 | | | |

: One-way ANOVA table for the tiller count data {#tbl-onewayresult .bordered}
...
Consider an experiment with $v$ levels of factor 1 and $r$ levels of factor 2. The observations are recorded as shown in @tbl-twoway. We have considered the situation of only one observation per cell.

```{r tbl-twoway,echo=FALSE,warning=FALSE,results='asis'}
#| tbl-cap: "Two-way classification of data"
...
library(knitr)
library(kableExtra)
dt <- read.csv("csv/twoway_classification.csv", check.names = FALSE)
colnames(dt) <- c("Factor A", "1", "2", "3", "$\\cdots$", "$r$", "Total")
dt %>%
  kbl(booktabs = TRUE, align = "c", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Georgia") %>%
  kable_styling(
  ...
  add_header_above(c(" " = 1, "Factor B" = 5, " " = 1), bold = TRUE)
```
...
| Source | D.F. | S.S | M.S.S | F |
|:-------|:----:|:----|:------|:--|
| Factor A | $v-1$ | $\dfrac{\sum_{i=1}^{v} T_i^2}{r} - \dfrac{G^2}{vr} = \text{SSA}$ | $\text{MSA} = \dfrac{\text{SSA}}{v-1}$ | $F_A = \dfrac{\text{MSA}}{\text{MSE}}$ |
| Factor B | $r-1$ | $\dfrac{\sum_{i=1}^{v} B_i^2}{v} - \dfrac{G^2}{vr} = \text{SSB}$ | $\text{MSB} = \dfrac{\text{SSB}}{r-1}$ | $F_B = \dfrac{\text{MSB}}{\text{MSE}}$ |
| Error | $(v-1)(r-1)$ | $\text{ESS} = \text{Total SS} - \text{SSA} - \text{SSB}$ | $\text{MSE} = \dfrac{\text{SSE}}{(v-1)(r-1)}$ | |
| Total | $vr-1$ | $\sum_{i=1}^{v}\sum_{j=1}^{r} y_{ij}^2 - \dfrac{G^2}{vr} = \text{Total SS}$ | | |

: Two-way ANOVA table {#tbl-twowaytable .bordered}
...
```{r tbl-twowayex,echo=FALSE,warning=FALSE,results='asis'}
#| tbl-cap: "Two-way classification of observations for chemicals and breeds"
...
library(knitr)
library(kableExtra)
dt <- read.csv("csv/twoway_data.csv", check.names = FALSE)
dt %>%
  kbl(booktabs = TRUE, align = "c", escape = FALSE) %>%
  kable_classic(full_width = F, html_font = "Georgia") %>%
  kable_styling(
  ...
  add_header_above(c(" " = 1, "Breeds of cow" = 5, " " = 1), bold = TRUE)
```
...
|:-------|:--:|:---:|:---:|:-----:|:-------------:|
| Factor A | 4 | 34.7 | 8.675 | $3.813^{*}$ | $F_{(4,12)}=3.26$ |
| Factor B | 3 | 32.2 | 10.73 | $4.7^{*}$ | $F_{(3,12)}=3.49$ |
| Error | 12 | 27.3 | 2.275 | | |
| Total | 19 | 94.2 | | | |

: ANOVA table for chemicals and breeds {#tbl-anovatabletwoway .bordered}
