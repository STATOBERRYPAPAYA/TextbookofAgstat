...
dt %>%
  ...
  add_header_above(c(" " = 1, "Observations" = 5, " " = 1), bold = FALSE)
```
...
```{r tbl-onewayanova, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
#| fig-cap: "One-way ANOVA table"
#| tbl-cap: "One-way ANOVA table"
#| filters:
#| - parse-latex
library(knitr)
library(kableExtra)
dt <- data.frame(
  Source = c("Between treatments", "Within treatments (Error)", "Total"),
  ...
  SS = c(
    "$\\dfrac{\\sum_{i=1}^{v} T_i^2}{r} - \\text{C.F.} = \\text{SST}$",
    "$\\text{TSS} - \\text{SST} = \\text{SSE}$",
    "$\\sum_{i=1}^{v}\\sum_{j=1}^{r} Y_{ij}^2 - \\dfrac{G^2}{n} = \\text{TSS}$"
  ),

  MSS = c(
    "$\\dfrac{\\text{SST}}{v-1} = \\text{MST}$",
    "$\\dfrac{\\text{SSE}}{v(r-1)} = \\text{MSE}$",
    ""
  ),
...
dt %>%
  ...
  add_header_above(c(" " = 1, "4 observations from each treatment" = 4, " " = 3), bold = FALSE)
```
...
```{r tbl-onewayresult, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
#| fig-cap: "One-way ANOVA table for the tiller count data"
#| tbl-cap: "One-way ANOVA table for the tiller count data"
#| filters:
#| - parse-latex
library(knitr)
library(kableExtra)
dt <- data.frame(
  ...
  MSS = c(
    "$\\dfrac{10427.21}{5}=2085.442$",
    "$\\dfrac{2932.74}{18}=162.93$",
    ""
  ),

  FCal = c(
    "$\\dfrac{2085.442}{162.93}=12.7996^{**}$",
    "",
    ""
  ),
...
colnames(dt) <- c("Source", "d.f", "S.S", "M.S.S", "F (Cal)", "F (tab)")
kbl(
  dt,
  booktabs = TRUE,
  align = "c",
...
```{r tbl-twowaytable,echo=FALSE,warning=FALSE,results='asis'}
#| fig-cap: "Two-way ANOVA table"
#| tbl-cap: "Two-way ANOVA table"
#| filters:
#| - parse-latex  
...
library(kableExtra)
dt <- data.frame(
  Source = c("Factor A", "Factor B", "Error", "Total"),
  ...
  SS = c(
    "$\\dfrac{\\sum_{i=1}^{v} T_i^2}{r} - \\dfrac{G^2}{vr} = \\text{SSA}$",
    "$\\dfrac{\\sum_{i=1}^{v} B_i^2}{v} - \\dfrac{G^2}{vr} = \\text{SSB}$",
    "$\\text{ESS} = \\text{Total SS} - \\text{SSA} - \\text{SSB}$",
    "$\\sum_{i=1}^{v}\\sum_{j=1}^{r} y_{ij}^2 - \\dfrac{G^2}{vr} = \\text{Total SS}$"
  ),

  MSS = c(
    "$\\text{MSA} = \\dfrac{\\text{SSA}}{v-1}$",
    "$\\text{MSB} = \\dfrac{\\text{SSB}}{r-1}$",
    "$\\text{MSE} = \\dfrac{\\text{SSE}}{(v-1)(r-1)}$",
    ""
  ),

  F = c(
    "$F_A = \\dfrac{\\text{MSA}}{\\text{MSE}}$",
    "$F_B = \\dfrac{\\text{MSB}}{\\text{MSE}}$",
    "",
    ""
  )
)
...
dt %>%
  ...
  add_header_above(c(" " = 1, "Breeds of cow" = 5, " " = 1), bold = FALSE)
```
