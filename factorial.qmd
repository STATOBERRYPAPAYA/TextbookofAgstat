...
When the number of factors or their levels increases, the treatment combinations grow rapidly. A $2^5$ factorial has 32 treatment combinations. To test these in a Randomised Complete Block Design (RCBD), every block would need 32 plots. Finding 32 plots uniform in soil fertility, moisture, and slope is very difficult, because the larger a block, the more variation it contains, and this variation inflates the experimental error and reduces precision.

The way out is to use smaller blocks. Instead of one big block of 32 plots, we split each replication into two smaller blocks of 16 plots each. Smaller blocks are more homogeneous, so the error goes down. But splitting comes at a price, and understanding that price is the whole idea of confounding.

### The core idea {.unnumbered}

To split the treatments into two blocks, we need a rule that decides which treatment combination goes into which block. We use one of the interaction contrasts to make this split. When we do that, the difference between the two block totals becomes exactly the same as that interaction's contrast so that the two are algebraically identical and cannot be separated. We can no longer tell whether a difference between the blocks is due to the blocks (soil, field position) or due to that interaction.

That interaction is said to be confounded with blocks *i*.*e* it is mixed up with the block effect and can no longer be estimated. In exchange, every other main effect and interaction is now estimated within smaller, more homogeneous blocks, so they are estimated more precisely.
...
Now look at what happened. From @sec-interaction, the interaction effect ABC is given by
...
So if Block 1 sits on richer soil than Block 2, that soil difference is inseparable from the ABC interaction, we have lost ABC. But the main effects A, B, C and the two-factor interactions AB, AC, BC are all balanced across the two blocks (each appears equally in both), so they are estimated cleanly, and now within blocks of 4 uniform plots instead of 8 uneven ones.
...
[**Partial confounding**]{.underline} — confound a *different* interaction in each replication. For example, confound ABC in replication I, AB in replication II, BC in replication III. Now ABC is confounded only in replication I, so it can still be estimated from replications II and III (where it is untouched). Every interaction is recovered from the replications where it is *not* confounded — at the cost of slightly reduced replication for each.
